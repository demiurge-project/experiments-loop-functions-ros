...
#ifndef EASTER_SW_LOOP_FUNC
#define EASTER_SW_LOOP_FUNC
...
class EasterSwLoopFunction: public CoreLoopFunctions {
  public:
    EasterSwLoopFunction();
    EasterSwLoopFunction(const EasterSwLoopFunction& orig);
    virtual ~EasterSwLoopFunction();
    ...
    void SetEggsOff();
    void SetEggColor(UInt32 unEggNumber, CColor ccEggColor);
  ...
  private:
    ...
    Real m_fRadius;
    Real m_fSecRadius;
    std::vector<UInt32> m_vEggOrder;
    UInt32 m_unActiveEgg;
    bool m_bTimeout;
    UInt32 m_unTimer;
    ...
    typedef std::vector<CVector2> TEggPositions;
    ...
    TEggPositions m_tEggPositions;

    //ROS stuff
    ros::Publisher odomPublisher[6];
    nav_msgs::Odometry odomMsg[6];
    ros::Publisher colorPublisher[6];
    std_msgs::ColorRGBA colorMsg[6];
