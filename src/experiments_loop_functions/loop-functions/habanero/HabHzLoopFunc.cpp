...
void HabHzLoopFunction::Init(TConfigurationNode& t_tree) {
    ...
    for (int i = 0; i < 8; ++i)
    {
      ...
      odomSubscriber[i] = rosNode.subscribe(ss.str(), 1, &HabHzLoopFunction::OdomCallback, this);  
    }
...
void HabHzLoopFunction::OdomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{   
    std::stringstream ss;
