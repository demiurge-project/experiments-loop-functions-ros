...
void HabRenLoopFunction::Init(TConfigurationNode& t_tree) {
    ...
    for (int i = 0; i < 8; ++i)
    {
      ...
      odomSubscriber[i] = rosNode.subscribe(ss.str(), 1, &HabRenLoopFunction::OdomCallback, this);  
    }
...
void HabRenLoopFunction::OdomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{   
    std::stringstream ss;
